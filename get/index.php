...
if (isset($json['motions']))
    foreach ($json['motions'] as $k => $v) foreach($v as $k2 => $v2) foreach ($v2 as $k3 => $path) {
        if ($k3 == 'file') $json['motions'][$k][$k2][$k3] = '../model/' . $modelName . '/' . $path;
        if ($k3 == 'sound') $json['motions'][$k][$k2][$k3] = '../model/' . $modelName . '/' . $path;
    }
